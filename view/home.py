...
class Home:
    def __init__(self, root):
        ...
        Label(self.root, image=self.logoPhotoImage).place(
            x=0, y=0, width=450, height=130)

        # second imag
        coverImg = Image.open(Images.coverPng).resize(
            (450, 130), Image.ANTIALIAS)
        self.coverPhotoImage = ImageTk.PhotoImage(coverImg)
        Label(self.root, image=self.coverPhotoImage).place(
        ...
        Label(self.root, image=self.smePhotoImg).place(
            x=900, y=0, width=450, height=130)

        # bg imag
        backgroundImg = Image.open(Images.background).resize(
